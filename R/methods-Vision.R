...
#' Run hotspot on this vision object.
#'
#' @export
#' @param object VISION object
#' @param counts 
...
setMethod("Hotspot", signature(object="Vision"), function(object, counts) {
            ...
            logexp <- log2(vis@exprData + 1)
            gi <- computeHotspotGiBinary(
              logexp, neighbors)
            
            # Takes a few minutes
            pvals <- gi_significance_all(
              gi, logexp, counts,
              neighbors, umis
            )
            
            # Select genes with significant hotspots
            THRESHOLD <- 0.1 / length(pvals)
            
            sig_genes <- rowAnys(pvals < THRESHOLD)
            sig_genes[is.na(sig_genes)] <- FALSE 
            ...
            gene_clusters <- data.frame(
              ClusterId = compkm$classification,
              MaxGi = rowMaxs(scaled_gi),
