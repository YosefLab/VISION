...
- Launching an interactive, web-based reort for viewing.
...
expression <- read.table('data/metastasis/cp007_expression.tsv.gz', sep='\t', header=T, row.names=1, stringsAsFactors = F)
...
# Read in meta data
meta <- read.table('data/metastasis/cp007_meta.tsv.gz', sep='\t', header=T, row.names=1)
...
A quick inspection of the meta data indicates that we have several types of data associated with each of the leaves of this tree. Not only do we have the anataomical site from which the cell was samples (`sampleID`) but also pre-computed statistics like the Metastatic Rate (`scTreeMetRate`). In fact, users can specify an arbitrary number of data items that have been precomputed. For example, if a user has performed fitness inference as in [Yang et al, bioRxiv 2021](https://www.biorxiv.org/content/10.1101/2021.10.12.464111v1), they can add these statistics as a column to the meta data file.
...
clusterByDepth <- function(tree, depth = 2) {
  ...
  stop("Targeted depth is larger than the depth of the tree. Repeat call with smaller target depth.")
}
...
vis <- PhyloVision(tree=tree, data=expression, signatures=sig, meta=meta, num_neighbors=30, projection_genes= projection_genes)
```
