...
file_path <- "..." # Replace the path
paste(file_path, "expr.tsv", sep="/")
expr <- read.table(paste(file_path, "expr.tsv", sep="/"), check.names = FALSE, sep = "\t")
umap <- read.table(paste(file_path, "umap_data.csv", sep="/"), check.names=FALSE, sep=",", row.names = 1, header=TRUE)
...
# Signature file
sigs <- paste(file_path, c("h.all.v5.2.symbols.gmt", "c6.all.v7.2.symbols.gmt", "c8.all.v7.2.symbols.gmt"), sep="/")

# Read the tree
tree <- read.tree(paste(file_path, "tree", sep="/"))
