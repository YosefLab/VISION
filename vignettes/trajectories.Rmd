...
Now, with this model saved we can pass it to VISION via the `latentTrajectory` parameter. The model will automatically be analzyed with the same signatures passed to the VISION object, and with this model, the `Trajectory` tab will be activated in the VISION output report. 
